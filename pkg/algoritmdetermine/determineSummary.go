...
func New(ctx context.Context, log *logrus.Logger, cfg *nt.ConfigDt, store *StoreMap.Brocker, in string, out []string) *DetermineSummarys {
...
func (d *DetermineSummarys) Run(ctx context.Context, ErrCh chan error) {
	d.stateRun = true
	...
	go func() {
		...
		d.Wg.Add(1)
		go d.Read(ctx, DoneInside, ErrCh)
		for {
			select {
			case <-DoneInside:
				{
	...
	for {
		...
		if data.Sc.Firstflag == 1 {
			...
			if resStr.Status == "save" {
				...
				if res := d.addSummaryStr(v.IdWell, &data.Sc.ResSheet); res != nil {
		...
		if countEpty > 100 {
			d.Log.Debug("DetermineSummarys all steams empty")
			return
		}
	}
}
